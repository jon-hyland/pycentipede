...
__break_chars = [" ", "-", "_", ".", "!", "?", "@", "$", "&", "*", ",", "[", "]", "(", ")", "{", "}", ";", ":", "%", "^", "~"]
__dictionary = di.dictionary
__split_cache = di.split_cache
__service_stats = di.service_stats


def simple_split(input_: str, cache: bool, max_terms: int, max_passes: int, errors: List[Exception]) -> SplitResult:
    ...
    try:
        ...
        # try from cache
        if cache:
            result = __split_cache.get_item(input_)
        else:
            result = None        

        # no? perform split logic
        if result is None:
            result = full_split(input_, cache, 1, max_terms, max_passes, errors)
...
def full_split(input_: str, cache: bool, pass_display: int, max_terms: int, max_passes: int, errors: List[Exception]) -> SplitResult:
    ...
    try:
        ...
        # execute split
        t = split_logic(input_, max_terms, max_passes)
        ...
        # cache
        if cache:
            __split_cache.set_item(input_, result)
        ...
        __service_stats.log_operation(name="full_split", elapsed_ms=sw.elapsed_ms)


def split_logic(input_: str, max_terms: int, max_passes: int) -> Tuple[List[Pass], List[Term]]:
    ...
    split_on_numbers(passes)

    # preserve strings like "a-1"
    preserve_a1(passes)

    # split on break chars
    split_on_break_chars(passes)

    # get small list of possible matching terms
    matched_terms: List[Term] = __dictionary.find_matching_terms(input_, 3)
    ...
    for p in passes:
        for s in p.splits:
            if not s.matched:
                best_term = __dictionary.find_term(s.text)
...
def split_on_numbers(passes: List[Pass]) -> None:
    """Split on numbers, with special cases."""
    ...
    for pass_ in passes:
        ...
        for term in __dictionary.get_special_numbers():
        ...
        for i in range(len(segments)):
            ...
            if not segment:
                continue
            if numeric_segment:
                term = __dictionary.find_term(segment)
                if term is not None:
...
def preserve_a1(passes: List[Pass]) -> None:
    ...
    for pass_ in passes:
        if (len(pass_.splits) >= 2) and (len(pass_.splits[0].text) == 2) \
            ...
            splits: List[Split] = []
            term = __dictionary.find_term(text)
            if term is not None:
                split = Split.from_term(term)
                splits.append(split)
            else:
...
def split_on_break_chars(passes: List[Pass]) -> None:
    ...
    for pass_ in passes:
        ...
        for split in pass_.splits:
            if (not split.matched) and (find_any(split.text, __break_chars) != -1):
                split_required = True
        if split_required:
            splits: List[Split] = []
            for split in pass_.splits:
                if split.matched or (find_any(split.text, __break_chars) == -1):
                    splits.append(split)
                else:
                    s = split.text
                    for c in __break_chars:
                        if c in s:
                            s = s.replace(c, " ")
                    items = s.split(" ")
                    for item in items:
